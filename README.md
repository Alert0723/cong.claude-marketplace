...
| `code-simplifier` | 全平台 | 代码简化 Agent - 提升代码清晰度、一致性和可维护性 |
...
代码简化 Agent，用于提升代码清晰度、一致性和可维护性，同时保持所有功能不变。

### 安装

```bash
/plugin install code-simplifier@cong.claude-marketplace
```

### 功能特性

code-simplifier Agent 自动分析并优化代码：

- **保持功能不变** - 只改变代码实现方式，不改变其行为
- **应用项目标准** - 遵循 CLAUDE.md 中定义的编码规范
- **提升清晰度** - 减少复杂性和嵌套，消除冗余代码
- **保持平衡** - 避免过度简化影响可读性
- **自动触发** - 代码修改后自动优化，无需显式请求
